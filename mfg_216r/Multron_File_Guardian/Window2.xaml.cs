    ...
    public partial class Window2 : Window
    {
        ...
        private void rjButtonp3_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    swillbesaved = 1;
                    process.parallelizm = short.Parse(textBoxp1.Text);
                    window.updatelbl();
        ...
        {
            if (MessageBox.Show("This will reset every settings in program, Do you still want to do?", "Multron File Guardian",  MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.Yes)
            {
                if (Directory.Exists(window.process.mfgsfolder) == true)
                {
                    Directory.Delete(window.process.mfgsfolder, true);
                }
                if (File.Exists(AppContext.BaseDirectory + "\\khfsdone.smfg") == true)
                {
                    File.Delete(AppContext.BaseDirectory + "\\khfsdone.smfg");
                }
                window.dntsave = 1;
                System.Diagnostics.Process.Start("cmd.exe", "/c timeout /t 3 /nobreak & start \"\" \"" + Process.GetCurrentProcess().MainModule.FileName + "\"");
                window.Close();
